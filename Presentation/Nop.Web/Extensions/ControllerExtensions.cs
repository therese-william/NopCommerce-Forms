            ...
            bool forceRedirectionAfterAddingToCart = false)
        {
            ...
            foreach (var product in products)
            {
                ...
                if (preparePriceModel)
                {
                    ...
                    {
                        priceModel.RetailPrice = priceFormatter.FormatPrice(product.RetailPrice.Value,true,false);
                    }
                    else
                    {
                        priceModel.RetailPrice = priceFormatter.FormatPrice(0,true,false);
                    }
                        ...
                        default:
                            {
                                ...
                                if (permissionService.Authorize(StandardPermissionProvider.DisplayPrices))
                                {
                                    //priceModel.RetailPrice = priceFormatter.FormatRentalProductPeriod(product, priceModel.RetailPrice);
                                    if (!product.CustomerEntersPrice)
                                    {
                                        if (product.CallForPrice)
                                        {
                                            ...
                                            priceModel.OldPrice = null;
                                            priceModel.Price = localizationService.GetResource("Products.CallForPrice");
                                            //priceModel.RetailPrice = localizationService.GetResource("Products.CallForPrice");
                                        }
                                        else
                                        {
                                            //prices
