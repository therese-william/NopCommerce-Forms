...
namespace Nop.Web.Controllers
{
    public partial class CatalogController : BasePublicController
    {
        ...
        [ChildActionOnly]
        public ActionResult ManufacturerTopNavigation()
        {
            var model = new List<ManufacturerModel>();
            var manufacturers = _manufacturerService.GetAllManufacturers();
            foreach (var manufacturer in manufacturers)
        ...
        public ActionResult Vendor(int vendorId, CatalogPagingFilteringModel command)
        {
            ...
            var vendorcity = _cityService.GetCityById(vendor.CityId.Value);
            var vendorstate = _stateProvinceService.GetStateProvinceById(vendor.StateId.Value);
            ...
            var model = new VendorModel
            {
                ...
                FullAddress = vendor.StreetAddress + ", " + vendorcity.CityName + ", "+vendorstate.Abbreviation + " " + vendorcity.PostCode 
            };
        ...
        [NopHttpsRequirement(SslRequirement.No)]
        public ActionResult VendorLocator(VendorLocatorModel locator)
        {
            if (locator == null)
            {
                locator = new VendorLocatorModel();
            }
            if (locator.SelectedCityId > 0)
            {
                var selectedCity = _cityService.GetCityById(locator.SelectedCityId);
                locator.SelectedCity = selectedCity;
                var locatedVendors = _vendorService.LocateVendors(selectedCity.Latitude.Value, selectedCity.Longitude.Value);
                foreach (var vendor in locatedVendors)
                {
                    var vendorcity = _cityService.GetCityById(vendor.CityId.Value);
                    var vendorstate = _stateProvinceService.GetStateProvinceById(vendor.StateId.Value);
                    locator.SelectedVendors.Add(new VendorModel
                    {
                        Id = vendor.Id,
                        Name = vendor.GetLocalized(x => x.Name),
                        Description = vendor.GetLocalized(x => x.Description),
            ...
            locator.AvailableCities = _cityService.GetCitiesForCombo();
            return View(locator);
        }
        ...
        [ChildActionOnly]
        public ActionResult VendorTopNavigation()
        {
            var model = new List<VendorModel>();
            var vendors = _vendorService.GetAllVendors();
            foreach (var vendor in vendors)
