...
namespace Nop.Admin.Validators.Vendors
{
    public class VendorValidator : BaseNopValidator<VendorModel>
    {
        public VendorValidator(ILocalizationService localizationService)
        {
            ...
            RuleFor(x => x.StreetAddress).NotEmpty().WithMessage(localizationService.GetResource("Admin.Vendors.Fields.StreetAddress.Required"));
            RuleFor(x => x.CityId).NotEmpty().WithMessage(localizationService.GetResource("Admin.Vendors.Fields.City.Required"));
        }
    }
}