...
namespace Nop.Services.Catalog
{
    ...
    public partial class PriceCalculationService : IPriceCalculationService
    {
            ...
            out Discount appliedDiscount)
        {
            ...
            //get dealer brand discount
            decimal DealerBrandDiscount = decimal.Zero;
            decimal DealerBrandDiscountValue = decimal.Zero;

            //VendorId, ManufaturerId
            var manufacturers = product.ProductManufacturers.ToList();

            foreach (ProductManufacturer m in manufacturers)
            {
                var vendorManufacturers = _manufacturerService.GetVendorManufacturersByVendorIdManufacturerId(customer.VendorId, m.ManufacturerId);
                if (vendorManufacturers != null && vendorManufacturers.Count > 0)
                {
                    foreach (VendorManufacturer vm in vendorManufacturers)
                    {
                        if (DealerBrandDiscount < vm.DiscountPercentage)
                        {
                            DealerBrandDiscount = vm.DiscountPercentage;
                        }
                    }
                }
            }

            if (DealerBrandDiscount > 0)
            {
                DealerBrandDiscountValue = (decimal)((((float)productPriceWithoutDiscount) * ((float)DealerBrandDiscount)) / 100f);
                appliedDiscountAmount += DealerBrandDiscountValue;
            }
